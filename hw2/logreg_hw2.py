...
def main():
  ...
  kagglew, kaggleloss = trainLogistic(X_train_bias, y_train, max_iters=max_iters, step_size=step_size)
  biasedTest = dummyAugment(X_test)
  prediction = biasedTest@kagglew >= 0
  kaggleresults = [["id", "type"]]
  count = 0

  for a in prediction:
        
    if a == True:
          kaggleresults.append([count, 1])
    else:
          kaggleresults.append([count, 0])
    
    count = count + 1
    
  np.savetxt('test_predicted.csv', np.array(kaggleresults), fmt='%s', delimiter=',')
...
def calculateNegativeLogLikelihood(X,y,w):
      ...
      for row in range(0, rows):
        # first chunk of equation
        temp1 = np.transpose(w)
        temp2 = np.matmul(temp1, X[row])
        temp3 = logistic(temp2)
        temp4 = np.log(temp3 + 0.00001)
        temp5 = y[row] * temp4

        # second chunk of equation
        temp6 = np.log(1 - temp3 + 0.00001) 
        temp7 = 1 - y[row]
        temp8 = temp7 * temp6
...
def trainLogistic(X,y, max_iters=max_iters, step_size=step_size):
    ...
    for i in range(max_iters):
          ...
          temp2 = logistic(temp1)
          # subtract by y
          temp3 = np.subtract(temp2, y)
          # transpose X
          temp4 = np.transpose(X)
          # multiply X^T with temp3
          temp5 = np.dot(temp4, temp3)
          # store in w_grad variable
          w_grad = temp5
...
def dummyAugment(X):
      n = np.shape(X)
      onescolumn = np.ones(n)
      aug_X = np.hstack((X, onescolumn))
      return aug_X
