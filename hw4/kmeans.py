...
def toyProblem():
  ...
  for x in range(0, 50):  
    centroids, assignments, sse = kMeansClustering(X, k=k, max_iters=max_iters, min_size=0, visualize=False)
    SSE_rand.append(sse[len(sse)-1])
...
def initalizeCentroids(dataset, k):
  ...
  centroids = randomgenerator.choice(dataset, size=k, replace=False)
...
def computeAssignments(dataset, centroids):
  ...
  if (len(centroids) == 1):
    assignments = np.zeros(len(dataset), dtype=int)
  
  else:
    size = np.shape(dataset)[1]
    assignments = np.linalg.norm(dataset[:,None] - centroids, axis=2)
    assignments = argpartition(assignments, 1, axis=1)
...
def updateCentroids(dataset, centroids, assignments):
  row_n, col_d = dataset.shape
  row_k, col_d2 = centroids.shape
  counts = np.zeros((row_k, 1))
  sums = np.zeros((row_k, col_d2))

  for x in range(0, row_n):
        centroid_id = assignments[x]
        sums[centroid_id] += dataset[x]
        counts[centroid_id] += 1
  
  for y in range(0, row_k):
        centroids[y] = sums[y] / counts[y]
...
def calculateSSE(dataset, centroids, assignments):
  ...
  sse = 0;
  k_row, d_col = centroids.shape

  for x,y in enumerate(dataset):
        cluster = centroids[assignments[x]]
        distance = 0

        for z in range(len(y)):
              calculation = (cluster[z] - y[z])**2
