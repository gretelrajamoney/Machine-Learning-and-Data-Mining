...
def main():
  ...
  # Q7 Evaluate on Test
  ################################
  kaggleValidation(net, X_test, batch_size)
...
def kaggleValidation(model, X_test, batch_size):
    
  j = 0
  count = 0
  kaggleresults = [["id", "digit"]]
  ...
  while j < len(X_test):
    ...
    p = np.argmax(logits,axis=1)[:,np.newaxis]

    for a in p:
      kaggleresults.append([count, a[0]])
      count = count + 1
